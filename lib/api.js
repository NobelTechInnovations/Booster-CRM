...
// Called whenever an authenticated request comes back 401 — the stored
// token is invalid or (far more often) has simply expired (7-day expiry,
// see signAuthToken on the backend). Previously every page just showed
// whatever raw error text it happened to catch and sat there, logged in
// to nothing; this clears the stale session and sends the user back to
// login instead, with a flag the login page uses to explain why they're
// suddenly there. A no-op if there was no session to begin with (a failed
// login attempt, or an already-public page) — nothing "expired" in that case.
function handleUnauthorized() {
  if (typeof window === "undefined") return;
  const hadSession = Boolean(getSession()?.token);
  clearSession();
...
// Auth endpoints intentionally return 401 for expected reasons (wrong
// password) that are just a form error, not "your session expired" —
// never redirect away from those.
...
export async function listAdCampaigns(channelId) {
  return request(`/api/ads/${channelId}/campaigns`, { headers: authHeaders() });
}

export async function listAdSets(channelId, campaignId) {
  return request(`/api/ads/${channelId}/campaigns/${campaignId}/adsets`, { headers: authHeaders() });
}

export async function listAds(channelId, adSetId) {
  return request(`/api/ads/${channelId}/adsets/${adSetId}/ads`, { headers: authHeaders() });
}

export async function updateAdCampaign(channelId, campaignId, payload) {
  return request(`/api/ads/${channelId}/campaigns/${campaignId}`, { method: "PATCH", headers: authHeaders(), body: JSON.stringify(payload) });
}

export async function updateAdSet(channelId, adSetId, payload) {
  return request(`/api/ads/${channelId}/adsets/${adSetId}`, { method: "PATCH", headers: authHeaders(), body: JSON.stringify(payload) });
}

export async function updateAdStatus(channelId, adId, status) {
  return request(`/api/ads/${channelId}/ads/${adId}`, { method: "PATCH", headers: authHeaders(), body: JSON.stringify({ status }) });
}

// Multipart — an optional new creative image alongside the caption, same
// raw-fetch-with-FormData pattern as every other file upload in this app.
export async function updateAdCreative(channelId, adId, { caption, imageFile }) {
  const form = new FormData();
  if (caption !== undefined) form.append("caption", caption);
  if (imageFile) form.append("image", imageFile);
  const response = await fetch(`${API_URL}/api/ads/${channelId}/ads/${adId}/creative`, {
    method: "PUT",
    credentials: "include",
    headers: authHeaders(),
    body: form,
  ...
  if (!response.ok) throw new Error(body.message || "Failed to update creative");
  return body;
}
...
// headerParams (a template's HEADER can carry its own {{1}}, independent
// of the body's) and buttonParams ({index, value} pairs — one per
// dynamic-URL button, e.g. a "Shop Now" link ending in .../{{1}}) are
// separate component slots Meta requires their own entries for; omitting
// either when the template actually has one is what "(#131008) Required
// parameter is missing" means.
export async function startWhatsAppTemplateConversation(to, templateName, language, bodyParams = [], bodyText = "", { headerParams = [], buttonParams = [] } = {}) {
  return request("/api/whatsapp/conversations/start-template", {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({ to, templateName, language, bodyParams, headerParams, buttonParams, bodyText }),
  });
}
...
// Shared by every support-ticket write that can carry file attachments —
// raw fetch with FormData, not the shared request() helper, for the same
// reason uploadWhatsAppMedia is: multipart/form-data needs the browser to
// set its own Content-Type (with boundary), not the JSON one request()
// always adds. Works fine with zero files too (a form with no file parts
// is still valid multipart), so callers never need two separate code paths
// depending on whether an attachment was picked.
async function requestMultipart(path, { fields = {}, files = [], headers = {} } = {}) {
  const form = new FormData();
  ...
  const body = await response.json().catch(() => ({}));
  // Public (no-login) ticket routes never return 401 — this only fires for
  // the one authenticated caller (the staff reply route), and handleUnauthorized
  // itself no-ops when there was never a session (i.e. every public call).
  if (response.status === 401) handleUnauthorized();
...
// files: up to 3 images/videos/PDFs, 12MB each (see utils/upload.js on the
// backend for the exact limits) — optional, a photo of a damaged product
// or a short clip is the expected use, not a hard requirement.
export async function createSupportTicket(companySlug, payload, files = []) {
  return requestMultipart(`/api/public/support/${encodeURIComponent(companySlug)}/tickets`, { fields: payload, files });
}
...
export async function commentOnPublicTicket(companySlug, ticketId, { phone, email, message, files = [] }) {
  return requestMultipart(`/api/public/support/${encodeURIComponent(companySlug)}/tickets/${encodeURIComponent(ticketId)}/comment`, {
    fields: { phone, email, message },
    files,
  });
}

// Public attachment view/download URL — phone/email as query params, the
// exact same re-validation contract as every other public ticket lookup.
export function publicSupportAttachmentUrl(companySlug, ticketId, attachmentId, { phone, email } = {}) {
  return `${API_URL}/api/public/support/${encodeURIComponent(companySlug)}/tickets/${encodeURIComponent(ticketId)}/attachments/${encodeURIComponent(attachmentId)}?${contactQuery(phone, email)}`;
}
...
export async function replySupportTicket(ticketId, message, files = []) {
  return requestMultipart(`/api/support/tickets/${ticketId}/reply`, {
    fields: { message },
    files,
    headers: authHeaders(),
  });
...
// Staff attachment view/download URL — carries the auth token as a query
// param since <img src>/<video src> can't set an Authorization header,
// same pattern as whatsappMediaUrl above.
export function staffSupportAttachmentUrl(ticketId, attachmentId) {
  const session = getSession();
  const token = session?.token ? `?token=${encodeURIComponent(session.token)}` : "";
  return `${API_URL}/api/support/tickets/${ticketId}/attachments/${attachmentId}${token}`;
}
