...
// "Continue with Facebook" — the Embedded Signup popup hands the frontend
// an authorization code plus the phone number Meta just set up; this
// exchanges that for a working connection with no token/ID typing.
export async function completeWhatsAppEmbeddedSignup({ code, phoneNumberId, whatsappBusinessAccountId }) {
  return request("/api/whatsapp/embedded-signup", {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({ code, phoneNumberId, whatsappBusinessAccountId }),
  });
}
