...
export async function copyStoreData({ sourceChannelId, targetChannelId }) {
...
// in order.
export async function startWhatsAppTemplateConversation(to, templateName, language, bodyParams = []) {
  return request("/api/whatsapp/conversations/start-template", {
    method: "POST",
    headers: authHeaders(),
    body: JSON.stringify({ to, templateName, language, bodyParams }),
  });
}
...
export async function listPublicOrdersByPhone(companySlug, phone) {
  return request(`/api/public/track/${encodeURIComponent(companySlug)}/orders?phone=${encodeURIComponent(phone)}`);
}

export async function getPublicOrderDetail(companySlug, orderId, phone) {
  return request(`/api/public/track/${encodeURIComponent(companySlug)}/orders/${encodeURIComponent(orderId)}?phone=${encodeURIComponent(phone)}`);
}
